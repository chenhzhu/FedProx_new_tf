...
class Model(object):
    ...
    def create_model(self, optimizer):
        ...
        fc1 = tf.compat.v1.keras.layers.Dense(inputs=outputs[:,-1,:], units=30)
        pred = tf.squeeze(tf.compat.v1.keras.layers.Dense(inputs=fc1, units=1))
        
        loss = tf.compat.v1.losses.sigmoid_cross_entropy(multi_class_labels=labels, logits=pred)
        #optimizer = tf.train.AdamOptimizer(learning_rate=self.lr)
        grads_and_vars = optimizer.compute_gradients(loss)
        grads, _ = zip(*grads_and_vars)
        train_op = optimizer.apply_gradients(grads_and_vars, global_step=tf.compat.v1.train.get_global_step())
        
        correct_pred = tf.equal(tf.cast(tf.greater(pred,0), tf.int64), labels)
